...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table, List<Map<String, String>> columns) {
        ...
        map.put("classname", Character.toLowerCase(tableEntity.getClassName().charAt(0))
                             + tableEntity.getClassName().substring(1));
        map.put("pathName", tableEntity.getClassname().toLowerCase());
        map.put("columns", tableEntity.getColumns());
        map.put("package", PropertiesUtil.getString(config, "package", "com.wlgroup"));
        map.put("author", PropertiesUtil.getString(config, "author", "eric"));
        map.put("email", PropertiesUtil.getString(config, "email", "zhangmingming@wulingd.com"));
        map.put("datetime", DateFormatUtil.DEFAULT_ON_SECOND_FORMAT.format(new Date()));
    ...
    public static String getFileName(String template, String className,
        ...
        if (template.contains("DTO.java.vm")) {
            return packagePath + "domain" + File.separator + className + "DTO.java";
        }
        ...
        if (template.contains("add.vue.vm")) {
            return packagePath + "vue" + File.separator + className.toLowerCase() + "-add.vue";
        }

        if (template.contains("list.vue.vm")) {
            return packagePath + "vue" + File.separator + className.toLowerCase() + "-list.vue";
        }

        if (template.contains("edit.vue.vm")) {
            return packagePath + "vue" + File.separator + className.toLowerCase() + "-edit.vue";
        }

        if (template.contains("detail.vue.vm")) {
            return packagePath + "vue" + File.separator + className.toLowerCase() + "-detail.java";
        }
